...
>
> **What E12 did NOT do, and it is deliberate.** No reorder fix: under hypothesis A the ordering code is correct and was merely raced, so `patch_etsy_listing`'s reconcile-after-upload comment stands unedited. One change, one observable — if a permutation ever recurs with the guard in place, the next reading is not confounded.
>
...
> GL-69, GL-71.** Closed by E11: **GL-68** (the drafter now receives the master
> artwork plus a sanitised `art_brief`, with an explicit "the artwork wins over
> the brief" rule) and **GL-70** (all three halves — `copy_only` no longer grades
> pixels it may not touch, the critic's reason now reaches the drafter and is
> persisted, and `hand_back_to_owner` replaces `abandon_candidate` on that path).
> Off-gate rows closed: **GL-72**. Merged: PR #15
> (`e11-copy-and-gallery-integrity` → master `ac715a3`, squash), **803 green**
> (790 baseline + 13 new). Findings: `docs/2026-08-12-e11-findings.md`.
> Next session: `docs/2026-08-13-e12-kickoff.md`.
>
> **The two rows still open on the gate are open for the same reason, and it is
> not a coding reason: neither can be closed without a live Etsy write.**
> **GL-69 is half closed** — `upload_listing_image` takes `alt_text` and refuses
> to upload an image whose alt text can never be set, and the secondary-group
> gap is filled by a title-derived `fallback_alt_text` written back to the row so
> a read-back has something real to compare against. What has *not* happened is
> the measurement the E11 PRD made non-negotiable: upload one image to a
> throwaway draft and read `alt_text` back. **`CLAUDE.md`'s own convention —
> verify by measurement, not by status code, earned by GL-22a Q2 — means this row
> does not close on a 200.** Nothing may re-upload listing `4554354628`'s gallery
> before that read-back lands. If the field is ignored, the entire fix surface is
> `upload_listing_image`; nothing else depends on it.
> **GL-71 is untouched by design, and that was the correct call.** The fix for
> hypothesis B is a two-line move and it stays unwritten until the cause is
> named with evidence, because shipping a reorder next to a dispatch dedupe makes
> the next permutation uninterpretable — and shipping a fix for an unmeasured
> cause is the same mistake in one commit instead of two. Discriminate first:
> read the listing's images *between* the upload loop and the reconcile in
> `group_product.patch_etsy_listing`. **If it lands on hypothesis A instead
> (duplicate `approve` dispatch racing the uploads), the `handle_decision`
> dispatch guard enters scope and GL-74 gets more urgent with it** — a
> single-tap irreversible reject next to a "tap until it lands" operating
> procedure.
>
> **One piece of debt this session created, recorded so it is not read as noise
> later.** `generate_draft_text` falls back to the text-only call, with a printed
> warning, if `base_image_local_path` is set but the file is gone — blind drafting
> is the GL-68 defect itself, so it is never silent, but a missing master is a
> storage fault rather than a reason to fail a draft. **If that warning ever
> appears in a real run, read it as a GL-69-class finding, not as log noise.**
> A test-seam note worth keeping: `compliance_draft` and `critic_pass` import the
> same `anthropic_client` module object, so patching `complete_with_images` "in
> one place" patches both — `_VisionDispatch` in `tests/test_critic_pass.py`
> routes on the prompt.
>
> **Board hygiene, discharged.** E11 deliberately left the GL-68/69/70/72 cells
> un-updated rather than fold them into a diff that already carried unreviewed
> pre-E11 planning edits — the right call, and the reason this pass exists. Those
> pending edits and this session's row updates land together here, with the §4
> caveat carried onto GL-69's cell. **This is the fourth consecutive pass to find
> or prevent prose/cell drift; the rule stands — move both sides or move neither.**
>
> **Standing owner decision 2026-08-12, and it retires a framing this board has
> been carrying since the morning: activation is not a planning variable.** No
> listing is activated until the pipeline is fully clear **and** the shop is out
> of developer mode. Publishing is the owner's call and the owner's timing — it
> is not gated on any row here, it is never a step in a session plan, and it is
> not to be raised as a question, a recommendation or a reminder. **The
> 2026-08-12 morning decision to "hold the first activation until GL-69 and
> GL-71 land" is therefore moot rather than satisfied:** everything stays a
> draft, so gallery and alt-text repair stay free indefinitely, and the urgency
> that decision created was misplaced. Recorded in `CLAUDE.md` so it does not
> have to be re-litigated each pass. **What this changes technically: nothing on
> the gate, and GL-69/GL-71 lose their deadline, not their content.**
>
> **GL-71 gains a confirmation step in front of its investigation (owner,
> 2026-08-12).** Confirm the gallery on `4554354628` is *still* scrambled before
> spending a session on causes — the owner's manual GL-57 dashboard pass may
> already have reordered it, and E10c is one snapshot of one listing. If it
> reads clean, the row is downgraded and re-verified on the next upload, not
> closed on a single read. **And if the cause turns out to be duplicate taps
> (hypothesis A), the dispatch guard and GL-74 are fixed in that same session —
> pre-approved, no separate scoping pass.**
>
> **Gate arithmetic, re-run under the published rule** (37 `GL-` rows − 7
> sub-lettered riders − 2 struck = 28 gate items): GL-68 and GL-70 close, so
> **25 of 28**, open set **GL-11, GL-69, GL-71**. GL-72 is not a gate item and
> its closure does not move the denominator.

> **Last updated 2026-08-12 — the two rows that were the critical path are closed,
> and the board's shape changed underneath them. Gate: twenty-three of
> twenty-eight, re-derived from the table rather than inherited. Five open:
> GL-11, GL-68, GL-69, GL-70, GL-71.** Closed by E10: **GL-7** (all
> four DoD items given verdicts rather than pauses) and **GL-52** (a fresh create
> measured *and* confirmed by the owner's eye in the Gelato Design editor).
> **GL-65** is localised — per-callback loss upstream of Telegram's queue — and
> **parked by the owner**, to be revisited post-go-live. Merged: PR #14
> (`e10a-ack-and-cadence` → master `289c337`), 790 green. Findings, one artefact,
> §0–§14: `docs/2026-08-11-e10b-findings.md`.
>
> **The inherited count was wrong and could not be reconciled to the table, so it
> has been re-derived from the rows and the rule is published with it.** The 08-11
> revision corrected a count and named the discipline — move both sides or move
> neither — but the lineage it corrected (24 of 27) does not reconstruct from the
> blocker table under any rule this pass could find. **Rather than add to a number
> nobody can reproduce, here is the derivation, and it is arithmetic anyone can
> re-run:** the blocker table holds **37** `GL-` rows; subtract **7** sub-lettered
> rider rows filed as parts of a parent (GL-19b, GL-22a/b/c/d, GL-23b, GL-10d) and
> **2** struck from the gate (GL-29 cancelled 08-06, GL-12 moved post-launch
> 08-08); that leaves **28 gate items**, of which **5 are open** — GL-11, GL-68,
> GL-69, GL-70, GL-71. **23 of 28.** If a future pass disagrees, it should
> disagree with the *rule*, not with the number.
>
> **Re-deriving it exposed drift that had nothing to do with arithmetic, and this
> is the more useful half.** Eight rows — GL-55, GL-56, GL-57, GL-58, GL-59,
> GL-60, GL-61, GL-62 — closed on 08-11 and were marked done **in the header prose
> only, never in their own cells**, so the table read as though all eight were
> still open. That is precisely the failure GL-23b's row was kept on the board to
> prevent, recurring five revisions later. All eight are now reconciled in-cell.
> **A ninth was not a bookkeeping error at all: GL-36 was genuinely closed by
> E10c** — proven live, after fixing a defect the proof exposed — and no hand-off
> mentioned it. **A board nobody re-derives is a board nobody should trust, and
> that now has three instances rather than one.**
>
> **The direction of travel, read honestly.** Two rows that had been open for over
> a week are discharged; four new ones are opened, and the four are all the same
> defect family — the pipeline writes listing copy about artwork it has never seen,
> and ships galleries it has not finished describing. **The gate did not get
> closer. It got wider, and more accurate.**
>
> **The single most important sentence out of E10: the drafter has never seen the
> artwork (GL-68).** `build_draft_prompt` passes a four-word sanitised niche, an
> image count and the strings `"flat"`/`"lifestyle"` — not the images, not
> `art_brief`, which is already on the row it holds. It has been invisible for as
> long as art and niche happened to agree. GL-55's sanitiser made it visible by
> being correct, and it cost three good designs in one night. **The owner has
> deliberately left the two batch tasks disabled pending it, so this row — not
> GL-11 — is the gate on the pipeline running at all.**
>
> **GL-11 is the only row still on the gate that is not ours, and it is no longer
> load-bearing.** GL-37 settled that the Creativity Standards questions are not
> API-settable and the owner accepted the manual per-listing tick; the email sent
> 08-06 can only improve on that answer, never block it. Follow up in-thread
> ~08-20 if still silent. **Nothing external stands between this shop and its
> first sale.**
>
> **What does stand between them is one owner decision, now taken.** Every listing
> in the shop is `draft`; the first activation is the owner ticking "an AI
> generator" in the web editor, which is the same save that takes the listing
> live. Candidate 81's listing `4554354628` is complete and clean. **Owner
> decision 2026-08-12: hold that activation until GL-69 and GL-71 land**, because
> alt text and gallery rank are both set at image-upload time and Etsy v3 has no
> update-image endpoint — repairing either after activation means re-uploading the
> gallery of a live listing, while repairing them on a draft costs nothing. **A
> few days of not being live, bought against a permanently degraded first
> listing.**
>
> **Next: E11 — `docs/2026-08-12-e11-kickoff.md`.** One code session in three
> parts (the copy-integrity chain GL-68/GL-70/GL-72; the gallery-integrity pair
> GL-69/GL-71; the GL-75 restore of candidates 77/78/79), then the first
> activation. **PRD-grade and unsigned: it touches a live external account and
> exceeds one sitting, so `CLAUDE.md` §2 applies and no code starts before
> sign-off.**
...
## Part 4 (cont.) — Session log (2026-08-12 — planning pass, E10 integration → E11)

**E10 did what four planning passes said it would, and the board got wider
anyway.** GL-7 and GL-52 are closed — the two rows that had been *the* critical
path since 08-05 — and GL-65 is localised. That is the first pass in three weeks
where the critical path actually moved. **And the same session produced four new
blockers, all of them the same defect.** The pipeline writes listing copy about
artwork it has never seen (GL-68), ships a gallery whose alt text it generated
and discarded (GL-69), retries copy against a rubric that grades pixels it may
not touch (GL-70), and uploads that gallery in an order Etsy then permutes
(GL-71). Four symptoms, one shape: **every stage after `generate` is reasoning
about the artwork through a description of it rather than through the artwork.**
The critic is the one stage that receives the images, and the critic is the one
stage that was right all night.

**The gate arithmetic, and the attempt to do it correctly is what found the real
problem.** This pass set out to apply the 08-11 rule — move both sides or move
neither — to the inherited 24 of 27. It could not: **24 of 27 does not
reconstruct from the blocker table under any rule.** So rather than add four to a
number nobody can reproduce, the count was re-derived from the rows and the
derivation published in the header: 37 table rows, minus 7 sub-lettered riders,
minus 2 struck from the gate, gives **28 gate items, 5 open — 23 of 28.** A
future pass that disagrees should disagree with the rule, which is now written
down, rather than with the number, which never was.

**Re-deriving it exposed the thing worth carrying, and it is not arithmetic.**
**Eight rows closed on 08-11 were marked done in the header prose and never in
their own cells** — GL-55, GL-56, GL-57, GL-58, GL-59, GL-60, GL-61, GL-62 — so
the table has been reading as though all eight were open for a day and two
sessions. That is exactly the failure GL-23b's row was deliberately kept on this
board to prevent, recurring five revisions after the row that documents it. All
eight are reconciled in-cell now. **And a ninth was not bookkeeping: GL-36 was
genuinely closed by E10c** — the live reconcile proved it after fixing the
endpoint defect the proof exposed — **and no hand-off, findings section or
close-out mentioned it.** A row can close by side effect and nobody notices,
which is the same blindness in the opposite direction.

**The direction of travel, read honestly:** closing two rows that blocked one
live night, and opening four that affect every listing forever, is not a net gain
toward shipping. The gate got wider and more accurate at the same time. Both
halves of that are worth having.

**Where this pass disagrees with the hand-off, stated once.** The hand-off's
decision 3 says client-side capture is "the only proposed instrument that can
see" the GL-65 loss. That is true and it is also the wrong frame: **diagnosis is
not the only useful move on a defect whose consequence is cheap to remove.**
GL-31 (the stall reminder ping, already filed post-launch) makes a silently
stalled group re-announce itself; it explains nothing and costs almost nothing.
**Owner decision 2026-08-12: park GL-65 entirely, revisit post-go-live, and in
the interim tap repeatedly until a decision lands.** Recorded with one rider that
is not a re-litigation: ⚠️ **the workaround may be one of the two live causes of
GL-71.** The quadruple `approve:100` tap ran `handle_decision` up to four times
for one group, and duplicate dispatch racing the gallery upload is the findings
doc's own hypothesis A for the scrambled ranks. So "tap a lot" is safe for the
Gelato create — the idempotent create-or-reuse held under a genuine quadruple
retry, which is a better test than any planned one — and **unproven for the
gallery.** E11 measures which hypothesis is true before it fixes anything, and
the answer decides whether the workaround needs a guard.

**The sequencing insight this pass contributes, and it is a scheduling fact
rather than an opinion: alt text and rank are set at upload, and Etsy v3 has no
update-image endpoint.** Confirmed in code — `etsy_client.upload_listing_image`
builds a multipart body carrying `image` and `rank` and has no `alt_text`
parameter at all; `group_product.py:621` is its only caller. So the cost of
GL-69 and GL-71 is not fixed: on a **draft** it is one re-upload nobody sees, and
on a **live** listing it is new image ids and new URLs on something that already
has age and stats. Candidate 81's listing is a complete, clean draft *today*.
**That is a window, not a state** — it closes the moment the owner ticks the
AI-generator box, because the same save activates. **Owner decision 2026-08-12:
hold the activation until GL-69 and GL-71 land.** A few days of not being live,
bought against a permanently degraded first listing. This is the first time on
this board that a scheduling decision has been driven by an API's *absence* of an
endpoint rather than by the behaviour of one.

**One thing the hand-off recorded as small that is not small: `RESEARCH_MODE`.**
`consume-pending-only` is still set in `.env`, the two batch tasks are re-enabled,
and the kickoff's instruction to unset it afterwards has not been carried out.
Leaving it is *correct today* and for a reason the hand-off did not give: with
GL-68 open, researching new candidates manufactures more copy/art mismatches to
repair by hand, exactly as GL-55/56 did. **So the flag is no longer a leftover —
it is load-bearing, and it has an explicit removal trigger: unset it when GL-68
is merged and the GL-75 backlog is drained, not before.** Written onto the E11
kickoff as a step with an owner tick, so it cannot rot into a leftover a second
time.

**A recurrence worth naming, because it is the third instance of one pattern.**
GL-53's lesson was *an instruction in a prompt is a preference, not a control*.
GL-69 is the same failure in a comment: `check_forbidden_terms`'s docstring
asserts alt texts "go live on the listing", and that assertion is false — which
is precisely how a review that *added validation to alt text* failed to notice
that alt text is never sent. **The generalisation this board should carry
forward: a statement of behaviour that lives only in prose — prompt, docstring or
plan — is not a behaviour. If it matters, an assertion or a test says it in
code, next to the decision.** GL-53 earned that lesson for prompts; GL-69 shows
it applies to our own comments, and the E10c reconcile defect (§10) shows it
applies to tests too — four tests passed against a broken endpoint choice because
each mocked whichever function the code happened to call.

**Deliverables from this pass:** the board rows above (GL-68 through GL-75, the
GL-49 rider, the GL-7/GL-52/GL-65 closures) and
`docs/2026-08-12-e11-kickoff.md` — the E11 PRD and kickoff. **E11 is PRD-grade
and unsigned:** it touches a live external account (the Etsy gallery re-upload
and the activation) and exceeds one sitting, so `CLAUDE.md` §2 applies. Its §7
lists the three decisions still open. **The activation itself is a separate,
explicit owner action and is not delegated to the sign-off on the code work.**
