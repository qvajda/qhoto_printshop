# Etsy AI-assisted POD pipeline — spec v0.4.9 (Telegram admin/allowlist ID added)

Status: **All Decisions Needed items resolved (D1–D8a).** This revision
adds one small, additive static-configuration item — your Telegram
admin/allowlist user ID — on top of v0.4.8's aspect-ratio-grouped review
flow. Nothing else changes. Ready to build once the Etsy app is approved
and the primary size's Gelato templates are created.
...
1. **Trend research** — three inputs feeding a shared "trend candidates" pool
   ...
   - **Trending now scan (scheduled):** ✅ investigated — Google Trends has
     ...
     rationale and what would need to be true to revisit it.
...
- **M1 — build for real, run manually one item at a time:** build the
  ...
  **M1 must include manually exercising the Kill branch** (section 3,
  ...
  section 4's placeholder policy.
