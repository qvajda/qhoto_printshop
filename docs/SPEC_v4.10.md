# Etsy AI-assisted POD pipeline — spec v0.4.10 (Etsy demand-proxy endpoint verified live)

Status: **All Decisions Needed items resolved (D1–D8a).** The Etsy app is
now approved. This revision resolves an implementation-time gap flagged
while planning the research stage — which real Etsy endpoint backs the
"candidate-keyword listing counts/favorites" demand proxy from section 3
step 1 — by verifying it live against the real Etsy API rather than
guessing its shape. No design/behavior change; see section 3 step 1 and
the CHANGELOG for details, including a bug this surfaced in the
already-shipped `etsy_client.py` request-signing helper.
