"""One-off: add the art_brief column to an existing candidates table
(db/schema.sql's CREATE TABLE IF NOT EXISTS won't retroactively add columns
to a table that already exists). Safe to run against any DB, any number of
times - a no-op if the column is already present. Same pattern as
migrate_base_artwork_columns.py.
"""
import sqlite3
import sys
